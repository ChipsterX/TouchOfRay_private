...
public:
	...
	inline void	 SetPower(float val) { m_fPower = val; clamp(m_fPower, 0.f, m_fPowerMax); }
    inline float GetRadiation() const { return m_fRadiation; }
    inline float GetPsyHealth() const { return m_fPsyHealth; }
    inline float GetSatiety() const { return 1.0f; }
	inline float GetArtefReaction() const { return 0.0f; }
	inline float GetSleep() const	{return 1.0f;}	
	virtual void ChangeSleep (const float value){};
	inline float GetThirst() const	{return 1.0f;}	
	virtual void ChangeThirst(const float value){};	
	
	inline float GetKurr() const	{return 1.0f;}	
	...
	inline void	SetMaxPower(const float val) { m_fPowerMax = val; clamp(m_fPowerMax, 0.1f, 1.0f); clamp(m_fPower, 0.f, m_fPowerMax); };
...
public:
    ...
    inline SConditionChangeV& change_v() { return (m_change_v); }
};
