...
// Страшно такое добавлять, пусть пока что будет
			if (!_valid(Position())) {
...
bool CGameObject::use(IGameObject* obj)
{
    ...
    if (scriptObj && scriptObj->m_door)
    {
		...
		if (Actor())
		{
			...
			R_ASSERT(GEnv.ScriptEngine->functor("bind_back_to_the_roots.animation_door_open", m_functor));
			animation_door = m_functor();
			if (game_value_debug_function)
				Msg("# actor open door");
		}
    }
