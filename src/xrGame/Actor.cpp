...
void CActor::Die(IGameObject* who)
{
    ...
    if (IsGameTypeSingle())
    {
		...
		LPCSTR game_over1;
		LUA m_functor1;
		R_ASSERT(GEnv.ScriptEngine->functor("bind_back_to_the_roots.game_over_script", m_functor1));
...
float CActor::currentFOV()
{
    ...
    {
        return pWeapon->GetZoomFactor() * (0.55f);
    }
    else
    {
        return g_fov;
...
void CActor::UpdateArtefactsOnBeltAndOutfit()
{
    ...
    if (fArtReac > 0.6f)
    {
        LPCSTR _af_reac;
        LUA m_functor10;
        R_ASSERT(GEnv.ScriptEngine->functor("bind_artefact_reaction.artefact_reaction", m_functor10));
        _af_reac = m_functor10();
    }
...
/*
	case ALife::eArtefReactionRestoreSpeed:
    {
        res = conditions().V_ArtefReaction();

        for (auto& it : inventory().m_belt)
        {
            const auto artefact = smart_cast<CArtefact*>(it);
            if (artefact)
                res += artefact->m_fArtefReactionRestoreSpeed * artefact->GetCondition();
        }
        const auto outfit = GetOutfit();
        CHelmet* pHelmet = (CHelmet*)inventory().ItemFromSlot(HELMET_SLOT);
        res += ((outfit ? outfit->m_fArtefReactionRestoreSpeed : 0.f) + (pHelmet ? pHelmet->m_fArtefReactionRestoreSpeed : 0.f));
...
//                        if (artefact)
//                            {
//								res += (artefact->m_fKurrRestoreSpeed*artefact->GetCondition());
//                            }
                }
//            CCustomOutfit* outfit = GetOutfit();
//                if (outfit)
//                {
//                    res += outfit->m_fKurrRestoreSpeed;
//                }
        break;
    }				
