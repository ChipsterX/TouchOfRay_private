...
#include "bloodsuckcs_vampire_execute.h"
#include "../states/state_hide_from_point.h"
#include "bloodsuckcs_vampire_approach.h"
#include "bloodsuckcs_vampire_hide.h"

#define TEMPLATE_SPECIALIZATION template <\
	typename _Object\
>

#define CStateBloodsuckcsVampireAbstract CStateBloodsuckcsVampire<_Object>

#define RUN_AWAY_DISTANCE			50.f

TEMPLATE_SPECIALIZATION
CStateBloodsuckcsVampireAbstract::CStateBloodsuckcsVampire(_Object *obj) : inherited(obj)
{
	add_state	(eStateVampire_ApproachEnemy,	xr_new CStateBloodsuckcsVampireApproach<_Object> 	(obj));
	add_state	(eStateVampire_Execute,			xr_new CStateBloodsuckcsVampireExecute<_Object> 	(obj));
	add_state	(eStateVampire_RunAway,			xr_new CStateMonsterHideFromPoint<_Object> 		(obj));
	add_state	(eStateVampire_Hide,			xr_new CStateBloodsuckcsVampireHide<_Object> 		(obj));
}

TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::reinit()
{
	inherited::reinit	();
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::initialize()
{
	...
	object->sound().play						(CAI_Bloodsuckcs::eVampireStartHunt);
}

TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::reselect_state()
{
	u32 state_id = u32(-1);
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::check_force_state()
{
	// check if we can start execute
	if (prev_substate == eStateVampire_ApproachEnemy) {
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::finalize()
{
	inherited::finalize();
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::critical_finalize()
{
...
TEMPLATE_SPECIALIZATION
bool CStateBloodsuckcsVampireAbstract::check_start_conditions()
{
...
TEMPLATE_SPECIALIZATION
bool CStateBloodsuckcsVampireAbstract::check_completion()
{
	// если убежал
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::setup_substates()
{
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckcsVampireAbstract::remove_links	(IGObj* object)
{
	if (enemy == object)
		enemy					= 0;
}
...
#undef CStateBloodsuckcsVampireAbstract