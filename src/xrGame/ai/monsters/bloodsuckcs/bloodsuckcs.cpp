...
void CAI_Bloodsuckcs::LoadVampirePPEffector(LPCSTR section)
{
...
void CAI_Bloodsuckcs::ActivateVampireEffector()
{
	Actor()->Cameras().AddCamEffector(xr_new CVampireCameraEffectcs(6.0f, get_head_position(this), get_head_position(Actor())));
	Actor()->Cameras().AddPPEffector(xr_new CVampirePPEffectcs(pp_vampire_effector, 6.0f));
}
...
void CAI_Bloodsuckcs::Die(IGObj* who)
{
	inherited::Die(who);
	stop_invisible_predator();
}
