...
void CUIActorMenu::Show(bool status)
{
    ...
    if (status)
    {
		...
		if (pActor && g_hand_hide_inventory)
		{
			LPCSTR status1;
			LUA m_functor;
			R_ASSERT(GEnv.ScriptEngine->functor("bind_back_to_the_roots.actor_v_inv_status_true", m_functor));
			status1 = m_functor();
		}
        ...
        LPCSTR ui_artefact_panel1;
        LUA m_functor2;
        R_ASSERT(GEnv.ScriptEngine->functor("ui_artefact_panel.artefct_panel_flag1", m_functor2));
        ui_artefact_panel1 = m_functor2();
    ...
    else
    {
        ...
        LPCSTR ui_artefact_panel2;
        LUA m_functor3;
        R_ASSERT(GEnv.ScriptEngine->functor("ui_artefact_panel.artefct_panel_flag2", m_functor3));
        ui_artefact_panel2 = m_functor3();

		if (pActor && g_hand_hide_inventory)
		{
			LPCSTR status0;
			LUA m_functor;
			R_ASSERT(GEnv.ScriptEngine->functor("bind_back_to_the_roots.actor_v_inv_status_false", m_functor));
			status0 = m_functor();
		}
    }
...
//--' true = Актёр не идёт при открытом меню\\--'
bool CUIActorMenu::StopAnyMove() 
{
    switch (m_currMenuMode)
    {
...
CUIDragDropListEx* CUIActorMenu::GetListByType(EDDListType t)
{
    switch (t)
    {
        case iActorBag:    { if (m_currMenuMode == mmTrade) return m_pTradeActorBagList; else return m_pInventoryBagList; } break;
