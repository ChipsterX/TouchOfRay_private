...
void CUIMainIngameWnd::Init()
{
	...
	if (type_hud_token == 2)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_CMP);

	if (type_hud_token == 3)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_STS);
	
	if (type_hud_token == 4)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_CS);

    if (type_hud_token == 5)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_SOC);

    if (type_hud_token == 6)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_VV);

    if (type_hud_token == 7)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_LA);

    if (type_hud_token == 8)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_MIS);
	
	if (type_hud_token == 9)
        uiXml.Load(CONFIG_PATH, UI_PATH, UI_PATH_DEFAULT, MAININGAME_XML_PCH);
...
void CUIMainIngameWnd::Draw()
{
    ...
    // show IO icon
    bool IOActive = (FS.dwOpenCounter > 0) && !(psActorFlags.test(AF_BOOL_DISABLE_MINIMAP));
    if (IOActive)
...
void CUIMainIngameWnd::AnimateContacts(bool b_snd)
{
    ...
    if (b_snd && !UIZoneMap->disabled && !(psActorFlags.test(AF_BOOL_DISABLE_MINIMAP)))
