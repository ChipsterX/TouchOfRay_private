...
void CActorCondition::LoadCondition(LPCSTR entity_section)
{
	...
	m_fKurrCritical				= pSettings->r_float(section,"kurr_critical");
	clamp						(m_fKurrCritical, 0.0f, 1.0f);
	m_fV_Kurr					= pSettings->r_float(section,"kurr_v");		
	m_fV_KurrPower				= pSettings->r_float(section,"kurr_power_v");
	m_fV_KurrHealth			    = pSettings->r_float(section,"kurr_health_v");	
...
void CActorCondition::UpdateArtefReaction()
{
/*	
    if (!IsGameTypeSingle())
    {
        m_fDeltaPower += m_fV_ArtefReactionPower * m_fDeltaTime;
        return;
    }

    if (m_fArtefReaction > 0)
    {
        m_fArtefReaction -= m_fV_ArtefReaction * m_fDeltaTime;
        clamp(m_fArtefReaction, 0.0f, 1.0f);
    }

    float ArtefReaction_health_koef = (m_fArtefReaction - m_fArtefReactionCritical) /
        (m_fArtefReaction >= m_fArtefReactionCritical ? 1 - m_fArtefReactionCritical : m_fArtefReactionCritical);
    if (CanBeHarmed() && !psActorFlags.test(AF_GODMODE_RT))
    {
        m_fDeltaHealth += m_fV_ArtefReactionHealth * ArtefReaction_health_koef * m_fDeltaTime;
        m_fDeltaPower += m_fV_ArtefReactionPower * m_fArtefReaction * m_fDeltaTime;
    }
*/
}
...
void CActorCondition::ChangeArtefReaction(float value)
{
/*
    m_fArtefReaction += value;
    clamp(m_fArtefReaction, 0.0f, 1.0f);
    Msg("Artefact reaction summ: [%f]", m_fArtefReaction);
*/
}
