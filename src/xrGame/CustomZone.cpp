...
void CCustomZone::Load(LPCSTR section)
{
    ...
    if (m_zone_flags.test(eSpawnBlowoutArtefacts))
    {
        ...
        R_ASSERT2(!(m_wItemCount & 1), "Invalid number of parameters in string 'artefacts'!");
        m_wItemCount >>= 1;
...
void CCustomZone::SpawnArtefact()
{
    ...
    Center(pos);
    Level().spawn_item(
        *m_ArtefactSpawn[i].section, pos, (GEnv.isDedicatedServer) ? u32(-1) : ai_location().level_vertex_id(), ID());
}
