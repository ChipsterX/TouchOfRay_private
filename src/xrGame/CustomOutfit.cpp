...
//	m_fKurrRestoreSpeed		= READ_IF_EXISTS(pSettings, r_float, section, "kurr_restore_speed",   0.0f );	
...
void CCustomOutfit::OnMoveToRuck_LA()
{
	if (m_pInventory)
	{
		visorWetness_1_ = 0.f; //  -    
...
void CCustomOutfit::OnMoveToSlot_LA()
{
	if (m_pInventory)
	{
		visorWetnessSpreadValue_1_ = Random.randF(MIN_RDROOPS_SPREAD_VALUE, MAX_RDROOPS_SPREAD_VALUE);
...
void CCustomOutfit::save(NET_Packet &packet)
{
...
void CCustomOutfit::load(IReader &packet)
{
...
//	result |= process_if_exists(section, "artefact_reaction", &CInifile::r_float, m_fArtefReactionRestoreSpeed, test);
	result |= process_if_exists( section, "thirst_restore_speed",   &CInifile::r_float, m_fThirstRestoreSpeed,   test );
//	result |= process_if_exists( section, "kurr_restore_speed",   &CInifile::r_float, m_fKurrRestoreSpeed,   test );	
