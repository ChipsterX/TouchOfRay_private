...
ENGINE_API float		ps_r2_sun_lumscale = 1.35f;	
...
void IConsole_Command::InvalidSyntax()
{
    ...
    OnConsoleInvalidSyntax(false, "~ Invalid syntax in call to '%s'", cName);
    OnConsoleInvalidSyntax(true, "~ Valid arguments: %s", I);
}

void IConsole_Command::OnConsoleInvalidSyntax(bool bLastLine, const char* pszMsg, ...)
{
    m_bInited = true;
...
void IConsole_Command::Message(oaErrorType MessageType, const char* pszMsg)
{
    VERIFY(m_bInited);
...
void IConsole_Command::Message(oaErrorType MessageType, const char* pszMsg, va_list& mark)
{
    VERIFY(m_bInited);
...
extern float gamma_DX = 0;
extern float brightness_DX = 0;
extern float contrast_DX = 0;

class CCC_Gamma : public CCC_Float
{
public:
    CCC_Gamma(LPCSTR N, float* V) : CCC_Float(N, V, 0.f, 0.9f) {}
    virtual void Execute(LPCSTR args)
    {
        CCC_Float::Execute(args);
        GEnv.Render->setGamma(1.1f + gamma_DX);
        GEnv.Render->setBrightness(1.1f + brightness_DX);
        GEnv.Render->setContrast(1.1f + contrast_DX);
...
ENGINE_API float psHUD_FOV_def = 0.35f; 
...
ENGINE_API int ps_teleport_demo_record = 0;
ENGINE_API bool renderer_allow_override = false;
ENGINE_API int game_value_gamemtl = 1;
ENGINE_API int game_console_show = 1;
ENGINE_API int game_value_discord_status = 1;
ENGINE_API int ps_r__Supersample = 1;

BOOL advSettingsDiclaimerIsShown = 0;

int game_value_prefetch = 0;
...
void CCC_Register()
{
    ...
    CMD2(CCC_Gamma,                                     "gamma_DirectX",        &gamma_DX);
    CMD2(CCC_Gamma,                                     "brightness_DirectX",   &brightness_DX);
    CMD2(CCC_Gamma,                                     "contrast_DirectX",     &contrast_DX);
...
//    CMD3(CCC_Mask,                                      "rs_refresh_60hz",      &psDeviceFlags,     rsRefresh60hz);
	...
	CMD4(CCC_Integer,                                   "game_gamemtl_value",   &game_value_gamemtl, 0, 1);
	CMD4(CCC_Integer,                                   "game_console_show_value", &game_console_show, 0, 1);
	CMD4(CCC_Integer,                                   "game_discord_status_value", &game_value_discord_status, 0, 1);
    ...
    CMD4(CCC_Integer, "game_load_prefetch",      &game_value_prefetch, 0, 1);
};
