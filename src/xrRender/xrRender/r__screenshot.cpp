...
void CRender::ScreenshotImpl(ScreenshotMode mode, LPCSTR name, CMemoryWriter* memory_writer)
{
    ...
    case IRender::SM_NORMAL:
    {
        ...
        string_path buf;
#ifndef USE_DX10
        xr_sprintf(buf, sizeof buf, "ss_%s_%s_(%s)_DX11.jpg", Core.UserName, timestamp(t_stemp),
                   g_pGameLevel ? g_pGameLevel->name().c_str() : "mainmenu");
#else
        xr_sprintf(buf, sizeof buf, "ss_%s_%s_(%s)_DX10.jpg", Core.UserName, timestamp(t_stemp),
            g_pGameLevel ? g_pGameLevel->name().c_str() : "mainmenu");
#endif
        ID3DBlob* saved = nullptr;
#ifdef USE_DX11
        ...
        { // hq
#ifndef USE_DX10
            xr_sprintf(buf, sizeof buf, "ssq_%s_%s_(%s)_DX11.tga", Core.UserName, timestamp(t_stemp),
                       g_pGameLevel ? g_pGameLevel->name().c_str() : "mainmenu");
#else
            xr_sprintf(buf, sizeof buf, "ssq_%s_%s_(%s)_DX10.tga", Core.UserName, timestamp(t_stemp),
                g_pGameLevel ? g_pGameLevel->name().c_str() : "mainmenu");
#endif
...
void CRender::ScreenshotImpl(ScreenshotMode mode, LPCSTR name, CMemoryWriter* memory_writer)
{
    ...
    case IRender::SM_NORMAL:
    {
        string64 t_stemp;
        string_path buf;
        xr_sprintf(buf, sizeof buf, "ss_%s_%s_(%s)_DX9.jpg", Core.UserName, timestamp(t_stemp),
                   g_pGameLevel ? g_pGameLevel->name().c_str() : "mainmenu");
        ID3DXBuffer* saved = nullptr;
        ...
        if (strstr(Core.Params, "-ss_tga"))
        { // hq
            xr_sprintf(buf, sizeof(buf), "ssq_%s_%s_(%s)_DX9.tga", Core.UserName, timestamp(t_stemp),
                       (g_pGameLevel) ? g_pGameLevel->name().c_str() : "mainmenu");
            ID3DXBuffer* saved = nullptr;
