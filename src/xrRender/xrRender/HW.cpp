...
void CHW::CreateDevice(HWND m_hWnd, bool move_window)
{
    ...
    P.BackBufferCount = ps_r3_backbuffers_count;
#else
	P.BackBufferCount = 1;
#endif
...
/*
    * A number that is always greater than the selected token
    * If it is less than the option, it will cause errors
*/
#define AllowedNumberOfTokens 12

u32 CHW::selectRefresh(u32 dwWidth, u32 dwHeight, D3DFORMAT fmt)
{
    /*
        * oldSerpski stalker
        * There is no special need for this option
        * Since DX9 will always inherit Hz from the monitor
    */

    if (ps_mHToken < AllowedNumberOfTokens)
