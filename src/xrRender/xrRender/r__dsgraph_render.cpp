...
template <class T> inline bool cmp_second_ssa(const T &lhs, const T &rhs) { return (lhs->second.ssa > rhs->second.ssa); }
template <class T> inline bool cmp_ssa       (const T &lhs, const T &rhs) { return (lhs.ssa         > rhs.ssa        ); }

template <class T> inline bool cmp_ps_second_ssa(const T &lhs, const T &rhs)
{
#ifdef USE_DX11
                                ...
                                for (auto &tex_it : nrmTextures)
                                {
                                    ...
                                    for (auto &it_it : items)
                                    {
                                        ...
                                        if (RCache.LOD.c_LOD)
										{
											...
											Fmatrix m;
                                            it_it.pVisual->getVisData().box.xform(m);
                        ...
                        for (auto &state_it : matStates)
                        {
                            ...
                            for (auto &tex_it : matTextures)
                            {
                                ...
                                for (auto &ni_it : items)
                                {
                                    ...
                                    if (RCache.LOD.c_LOD)
									{
										Fvector o_position = ni_it.pObject->GetRenderData().xform.c;
