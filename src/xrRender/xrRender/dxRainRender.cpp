...
dxRainRender::dxRainRender()
{
    if (ps_r3_rain_4p == 1)
    {
        IReader* F = FS.r_open("$game_meshes$", "render_rain\\rain_4p.dm");
        VERIFY3(F, "Can't open file.", "rain_4p.dm");
    ...
    {
        IReader* F = FS.r_open("$game_meshes$", "render_rain\\rain_12p.dm");
        VERIFY3(F, "Can't open file.", "rain_12p.dm");
...
//#pragma todo("qweasdd to himself: replace 40, 30,20 with sonsole command")
		...
		rain_params_new->x = saturate(rain_params_new->x);
		rain_params_new->y = saturate(rain_params_new->y);
		rain_params_new->z = saturate(rain_params_new->z);
    }
        ...
        if (wlen > b_radius_wrap_sqr)
        {
            ...
            else
            {
                ...
                if (src_plane.intersectRayPoint(one.P, inv_dir, src_p))
                {
                    ...
                    if (owner.RayPick(src_p, one.D, height, targetMaterial, collide::rqtBoth))
                    {
                        if (_sqr(height) <= dist_sqr)
                        {
                            one.invalidate(); // need born
                        ...
                        {
                            owner.RenewItem(one, height - _sqrt(dist_sqr), targetMaterial, TRUE);  // fly to point
                            //							Log("2",height-dist);
                        }
                    }
                    else
                    {
                        owner.RenewItem(one, max_distance - _sqrt(dist_sqr), targetMaterial, FALSE);
                        //						Log("3",1.5f*b_height-dist);
                    }
                }
                else
