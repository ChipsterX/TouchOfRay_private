...
void CParticleEffect::OnFrame(u32 frame_dt)
{
	...
	if (distance_from_camera < ps_r2_particle_distance || ps_r2_particle_distance >= 35){
...
void CParticleEffect::Render(float)
{
	...
	if (p_cnt>0 && distance_from_camera < ps_r2_particle_distance)
	{
        if (m_Def && m_Def->m_Flags.is(CPEDef::dfSprite))
        {
            ...
            if (dwCount)
            {
                ...
                if (GetHudMode())
                {
                    RDEVICE.mProject.build_projection(deg2rad(psHUD_FOV * Device.fFOV), Device.fASPECT, 0.05f,
