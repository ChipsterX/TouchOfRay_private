...
{
    // Clear depth & stencil
	u_setrt	( rt_Position, rt_Normal, rt_Color, rt_Extension, 0 );
	CHK_DX	( HW.pDevice->Clear	( 0L, NULL, D3DCLEAR_TARGET, 0x0, 1.0f, 0L) );
    //  Igor: soft particles

	u_setrt(Device.dwWidth, Device.dwHeight, rt_Position->pRT, NULL, NULL, NULL, HW.pBaseZB);
...
void CRenderTarget::phase_scene_begin()
{
    ...
    if (RImplementation.o.albedo_wo)
        u_setrt(rt_Position, rt_Normal, rt_Accumulator, rt_Extension, HW.pBaseZB);
    else
        u_setrt(rt_Position, rt_Normal, rt_Color, rt_Extension, HW.pBaseZB);
