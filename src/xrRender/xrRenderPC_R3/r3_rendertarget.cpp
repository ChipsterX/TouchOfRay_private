...
#include "xrRender/xrRenderDX10/dx10MinMaxSMBlender.h"
#include "xrRender/xrRenderDX10/msaa/dx10MSAABlender.h"
#include "xrRender/xrRenderDX10/DX10 Rain/dx10RainBlender.h"
#include <D3DX10Tex.h>
...
CRenderTarget::CRenderTarget()
{
	...
	b_dlaa = new CBlender_dlaa();
	b_rain_drops = new CBlender_rain_drops();
	b_rain_drops_diff = new CBlender_rain_drops_diff();
	b_gasmask = new CBlender_gasmask();
	b_gamma = new CBlender_gamma();
	b_vignette = new CBlender_vignette();	
	b_ssr = new CBlender_ssr();							  
#ifndef DISABLE_SSAO_SYSTEM_R3
    ...
    //	NORMAL
    {
        ...
        rt_Extension.create(r2_RT_E, w, h, D3DFMT_A8R8G8B8, SampleCount);
        ...
        if (RImplementation.o.dx10_msaa)
            rt_PPTemp.create("$user$genericpp", w, h, D3DFMT_A8R8G8B8, SampleCount);
        else
		...
		rt_ssr.create(r2_RT_ssr, w, h, D3DFMT_A8R8G8B8);
		rt_ssr_blur.create(r2_RT_ssr_blur, w, h, D3DFMT_A8R8G8B8);	
		
		/*		Rendertarget for future Ambient Occlusion			
		rt_ao.create(r2_RT_ao, u32(w / 2), u32(h / 2), D3DFMT_R16F);
		*/		
    ...
    // BLOOM
    {
        ...
        s_bloom.create(b_bloom, "DX9\\bloom");
        if (RImplementation.o.dx10_msaa)
        {
	...
	s_rain_drops_diff.create(b_rain_drops_diff);
	s_rain_drops_diff.create(b_rain_drops_diff);
    ...
    // Build textures
    {
        ...
        if (1)
        {
            ...
            for (u32 y = 0; y < TEX_jitter; y++)
            {
                for (u32 x = 0; x < TEX_jitter; x++)
                {
                    float numDir = 8.0f; //LVutner - We do not need to switch that with ssao quality
...
//_SHOW_REF("t_envmap_1 - #small",t_envmap_1->pSurface);
	...
	xr_delete(b_rain_drops_diff);
	xr_delete(b_gasmask);
	xr_delete(b_gamma);
	xr_delete(b_vignette);				   
	xr_delete(b_ssr);
    if (RImplementation.o.dx10_msaa)
    {
        int bound = RImplementation.o.dx10_msaa_samples;
