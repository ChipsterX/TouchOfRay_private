...
HRESULT CRender::shader_compile(LPCSTR name, char* shader, size_t shaderSize, LPCSTR pFunctionName,
    LPCSTR pTarget, DWORD Flags, void*& result)
{
    ...
    //sweetfx
    if (ps_r3_technicolor)
    {
        defines[def_it].Name = "USE_TECHNICOLOR";
        defines[def_it].Definition = "1";
        def_it++;
        sh_name[len] = '1'; ++len;
    }
    ...
    strconcat(sizeof(folder), folder, "r3\\objects\\DX11\\", name, ".", extension);
