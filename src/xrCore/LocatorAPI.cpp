...
static constexpr pcstr FSLTX = "database\\file_initializer\\file_initializer.ltx";
...
void CLocatorAPI::_initialize(u32 flags, pcstr target_folder, pcstr fs_name)
{
    ...
    //oldSerpski stalker
    {
        ...
        const unsigned _v1         = unsigned(LOBYTE(LOWORD(encodedVersion)));
        const unsigned _v2         = unsigned(HIBYTE(LOWORD(encodedVersion)));

        Log("* The entire list of possible operating systems");
        Log("* else 5.1 then Windows XP");
        Log("* else 6.0 then Windows Vista");
        Log("* else 6.1 then Windows 7");
        Log("* else 6.2 then Windows 8"); 
        Log("* else 6.3 then Windows 8.1");
        Log("* else 10 then Windows 10");

        Log("* Initializing system...");
        Msg("* Running under Windows %u.%u\n", _v1, _v2);
    }
    ...
    else
    {
        ...
        while (!pFSltx->eof())
        {
            ...
            CHECK_OR_EXIT(I.second,
                "The file 'database\\file_initializer\\file_initializer.ltx' is corrupted (it contains duplicated lines).\n"
    ...
    CreateLog(nullptr != strstr(Core.Params, "-no_log"), nullptr != strstr(Core.Params, "-no_log_overflow"));
}
