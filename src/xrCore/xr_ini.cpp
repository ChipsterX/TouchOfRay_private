...
XRCORE_API CInifile*       pSettings_not_const = nullptr;
    ...
    while (!F->eof())
    {
        ...
        else if (str[0] && str[0] == '[') // new section ?
        {
            ...
            if (Current)
            {
                ...
                {
                    Msg("------------------------------------------------------------------------");
                    Msg("! [DebugInfo]: [INFO]: WARNING C++!");
                    Msg("! [DebugInfo]: [INFO]: When reading configs, duplicate sections were found!");
                    Msg("! [DebugInfo]: [INFO]: Fix this section!");
                    Msg("! [DebugInfo]: [ERROR]: Duplicate section [%s] found.", *Current->Name);
                    Msg("! [DebugInfo]: [INFO]: If this is 'game_info', then you can skip this. So it has a systemic origin and cannot be settled.");
                    Msg("------------------------------------------------------------------------");
                }
...
CInifile::Sect& CInifile::r_section(pcstr S) const
{
    ...
    if (!(I != DATA.cend() && xr_strcmp(*(*I)->Name, section) == 0))
        xrDebug::Fatal(DEBUG_INFO, "Can't open section '%s'.", S);
    return **I;
}
