...
#endif
{
	...
	float puddles = s_puddles.Sample( smp_base, I.tcdh*128.f).r * mask.g;				
	puddles += ripples(I.tcdh.xy, 4096.f)*puddles;
	puddles *= saturate(ogse_c_rain.w);		
  ...
  O				= pack_gbuffer( 
								...
								float4	(0,	0, 0, puddles));	// OUT: rgb.gloss
