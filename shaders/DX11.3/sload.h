...
float4 tgloss_coef_and_offset; //gloss coeficient and offset for texture; assigned in textures_specifications.ltx
...
float4 parallax;		//defined in engine
float4 steep_parallax;	//defined in engine

static const float fParallaxStartFade = steep_parallax.y - 4;
static const float fParallaxStopFade = steep_parallax.y;

void UpdateTC( inout p_bumped I)
{
	if (I.position.z < fParallaxStopFade)
	{
		const float maxSamples = steep_parallax.z;
		const float minSamples = steep_parallax.w;
		const float fParallaxOffset = -1 * steep_parallax.x;
...
surface_bumped sload_i( p_bumped I)
{
	...
	S.gloss		= Nu.x*Nu.x * tgloss_coef_and_offset.x + tgloss_coef_and_offset.y;
	S.height	= NuE.z;
	//S.height	= 0;
