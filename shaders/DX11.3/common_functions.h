...
{
	#define  fWhiteIntensitySQR (1.7*1.7)
	rgb		=	rgb*sqrt(scale);
	...
	else  // Дезовейв\Лост алфа деволт блоом
	{
		low		= rgb.xyzz;
		high	= rgb.xyzz-saturate(rgb.xyzz);	// 8x dynamic range
	}
...
half2 gbuf_pack_normal (half3 n)
{
    half f = sqrt(8*n.z+8);
    return n.xy / f + 0.5;
}

half3 gbuf_unpack_normal (half2 enc)
{
    half2 fenc = enc*4-2;
...
f_deffer pack_gbuffer( float4 norm, float4 pos, float4 col,float4 ext )
#else
f_deffer pack_gbuffer( float4 norm, float4 pos, float4 col, float4 ext, uint imask )
#endif
{
	f_deffer res;
...
gbuffer_data gbuffer_load_data( float2 tc : TEXCOORD, float2 pos2d, int iSample )
{
   ...
   float4	E	= s_extension.Sample( smp_nofilter, tc );
	gbd.E		= E.xyz;
	gbd.puddles	= E.w;
